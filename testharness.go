...
// the same index. Is also verifies that all commands *before* cmd in
		...
		if h.connected[i] {
			if commitsLen >= 0 {
				...
				if len(h.commits[i]) != commitsLen {
					h.t.Fatalf("commits[%d] = %d, commitesLen = %d", i, h.commits[i], commitsLen)
				}
			} else {
...
func (h *Harness) CheckCommittedN(cmd int, n int) {
	...
	if nc != n {
		h.t.Errorf("CheckCommitedN got nc=%d, want %d", nc, n)
	}
}

// CheckNotCommitted verifies that no command equal to cmd has been committed
// by any of the active server yet.
...
// collectCommit reads channel commitChans[i] and adds all received entries
